...
import { Leaf, PawPrint, Recycle, Sparkles, Star, type LucideIcon } from 'lucide-react';
...
const INGREDIENTS: { icon: LucideIcon; title: string; desc: string }[] = [
  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: count }).map((_, i) => (
        <Star key={i} className="h-3.5 w-3.5 fill-[#C4A484] text-[#C4A484]" aria-hidden />
      ))}
    </div>
  );
}
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-16 px-6 lg:grid-cols-2 lg:px-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            {INGREDIENTS.map((item) => {
              ...
              >
                <Icon className="h-6 w-6 text-[#C4A484]" strokeWidth={1.5} aria-hidden />
