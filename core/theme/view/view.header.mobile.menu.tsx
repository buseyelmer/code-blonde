...
import { CloseIcon } from "@/theme/item/item.icons";
import { ItemLogo } from "@/theme/item/item.logo";

type ViewHeaderMobileMenuProps = {
  open: boolean;
  onClose: () => void;
};

export function ViewHeaderMobileMenu({ open, onClose }: ViewHeaderMobileMenuProps) {
    ...
    <div className="fixed inset-0 z-[60] md:hidden">
      ...
      <aside className="absolute right-0 top-0 flex h-full w-[min(100%,20rem)] flex-col bg-white shadow-xl">
        <div className="flex items-center justify-between gap-3 border-b border-stone/60 px-4 py-3.5">
          <ItemLogo variant="mobile" onNavigate={onClose} />
          <button
            type="button"
            onClick={onClose}
