...
import {
  ...
  MenuIcon,
  SearchIcon,
} from "@/theme/item/item.icons";
import { ViewHeaderAuthLinks } from "@/theme/view/view.header.auth.links";
import { ViewHeaderCartLink } from "@/theme/view/view.header.cart.link";
import { ViewHeaderMobileMenu } from "@/theme/view/view.header.mobile.menu";

export function SectionLayoutHeader() {
    ...
    <header className="max-w-full overflow-hidden border-b border-stone/50 bg-white">
      <div className="mx-auto flex min-h-[4.25rem] max-w-7xl items-center gap-2 px-3 py-2 sm:min-h-[5.25rem] sm:gap-4 sm:px-6 sm:py-2.5 lg:px-8">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-0.5 sm:gap-1">
          <ViewHeaderAuthLinks className="hidden md:flex" />
      ...
      <ViewHeaderMobileMenu open={menuOpen} onClose={() => setMenuOpen(false)} />
    </header>
  );
}
