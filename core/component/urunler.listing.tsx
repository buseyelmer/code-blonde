...
import {
  ...
  getCategoryHref,
  getCategoryName,
} from "@/core/util/category";
import "@/core/util/util";

const CLIENT_SORT_FETCH_AMOUNT = 200;

type ServerSortParams = {
...
function getSortFetchConfig(sort: SortOption): {
  clientSort: boolean;
  ...
  switch (sort) {
    case "popular":
      return {
        clientSort: false,
        params: { orderBy: "reviewCount", orderDirection: "desc", sortBy: "reviewCount" },
      };
    case "rating":
      return {
        clientSort: false,
        params: { orderBy: "avgRating", orderDirection: "desc", sortBy: "avgRating" },
      };
    case "bestsellers":
      return {
        clientSort: true,
    ...
    case "price-desc":
      return {
        clientSort: true,
        params: { order: { column: "createdAt", direction: "desc" } },
      };
    case "newest":
    default:
      return {
...
function buildListingHref(options: {
  ...
  search: string | null;
  categories: Parameters<typeof getCategoryHref>[0][];
}) {
  const searchQuery = options.search
    ? `?search=${encodeURIComponent(options.search)}`
    : "";

  if (!options.categoryId) return `/urunler${searchQuery}`;

  const category = options.categories.find((item) => item.id === options.categoryId);
  if (!category) return `/urunler${searchQuery}`;

  return `${getCategoryHref(category, options.categories)}${searchQuery}`;
}

export function UrunlerPageContent({ categorySlug }: { categorySlug?: string }) {
  ...
  useEffect(() => {
    ...
    if (legacyCategoryId && allCategories.length > 0) {
      const match = allCategories.find((item) => item.id === legacyCategoryId);
      if (match) {
        router.replace(buildListingHref({
          categoryId: match.id,
          search,
    ...
    const slugCategory = categorySlug
      ? findCategoryBySlug(allCategories, categorySlug)
      : undefined;

    setFilters((prev) => {
      const next = {
        ...prev,
        categoryId: slugCategory?.id ?? null,
        subCategoryId:
          slugCategory && prev.categoryId !== slugCategory.id ? null : prev.subCategoryId,
        search: search ?? null,
      };
      if (
  ...
  }, [searchParams, categorySlug, allCategories, router]);
  ...
  const activeCategory = useMemo(
    () => allCategories.find((item) => item.id === filters.categoryId) ?? null,
    [allCategories, filters.categoryId],
  );

  const handleFiltersChange = (next: ListingFilters) => {
    const categoryChanged = next.categoryId !== filters.categoryId;
    setFilters(next);
  ...
  const sortConfig = getSortFetchConfig(filters.sort);
  const useClientSort = sortConfig.clientSort;
  ...
  } as Parameters<ReturnType<typeof useProduct>["fetch"]>[0]);
  ...
  const pageTitle = activeCategory ? getCategoryName(activeCategory) : "Tüm Ürünler";
        ...
        <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-6 lg:px-8">
          <nav className="mb-3 flex items-center gap-2 text-xs text-[#8B6B57]">
            ...
            {activeCategory && (
              <>
                <span className="text-[#D9C5B0]">/</span>
                <span className="text-[#5C4638]">{getCategoryName(activeCategory)}</span>
              </>
            )}
          </nav>
